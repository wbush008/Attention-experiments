...
cd /Users/grad_user/Dropbox/Will_Experiments/GaborRep
% 
...
FileName =  strcat(ID, '_Rep_Gabor.csv'); 
...
fprintf(FID, 'Block, BlockOrder, Trial, TrialOrder, TrCueSide, TrStandOrient, TrCompContrast, TrStandSide, Response, Accuracy, RT, T1, T2, T3, T4\n'); %\n=enter - moves to next row
...
LeftCueLoc = [CX-140 CY-55 CX-130 CY-45];
RightCueLoc = [CX+130 CY-55 CX+140 CY-45];
CenterCueLoc = [CX-5 CY-5 CX+5 CY+5];
...
TotalBlock =14; % # of blocks
CueSide = 3; %1=left;2=right; 3=none
...
BlockTrial = CueSide*StandSide*StandOrient*CompContrast*RepetitionInBlock; % #=108*reps
PracTrial = 75;
...
for i=0 : BlockTrial-1
    ...
    TrCueSide(i+1) = mod(i/36, CueSide);
	TrCueSide(i+1) = floor(TrCueSide(i+1))+1;
...
if expType == 'e'
    ...
    StandGab = gabor([70 70], 12, 45, rand, 12 , 0.5, .22)*256;
    CompGab = gabor([70 70], 12, -45, rand, 12 , 0.5, .78)*256;
    ...
    Screen(window, 'FillRect', black, FixationLoc);
    StandGab = gabor([70 70], 12, 45, rand, 12 , 0.5, .78)*256;
    ...
    Screen(window, 'FillRect', grey);
        ...
        Screen(window, 'FillRect', black, FixationLoc);
        StandGab = gabor([70 70], 12, 45, rand, 12 , 0.5, .22)*256;
        CompGab = gabor([70 70], 12, -45, rand, 12 , 0.5, .06)*256;
        Screen('PutImage', window, StandGab, LeftLoc);
        Screen('PutImage', window, CompGab, RightLoc);
        Screen('Flip', window);
        WaitSecs(.5);  
        ...
        Screen(window, 'FillRect', black, FixationLoc);
        StandGab = gabor([70 70], 12, -45, rand, 12 , 0.5, .22)*256;
        CompGab = gabor([70 70], 12, 45, rand, 12 , 0.5, .09)*256;
    ...
    Screen('PutImage', window, imread('GaborDisplay2.png'), WholeScreen);
    Screen(window, 'FillRect', black, FixationLoc);
    Screen(window, 'FillOval', black, RightCueLoc);
    Screen('Flip', window);
    WaitSecs(2);
    KbWait(-1);
...
StandGab = gabor([70 70], 12, SOrient, rand, 12 , 0.5, .22)*256;
CompGab = gabor([70 70], 12, COrient, rand, 12 , 0.5, ContrastList(TrCompContrast(Order(Trial))))*256;
...
if TrCueSide(Order(Trial)) == 1  
    CueLocation = LeftCueLoc;
elseif TrCueSide(Order(Trial)) == 2   
    CueLocation = RightCueLoc;
else
    CueLocation = CenterCueLoc;
end    
...
Screen(window, 'FillRect', black, FixationLoc);
Screen(window, 'FillOval', CueColor, CueLocation);
[T1Timestamp Cue_Time]=Screen('Flip', window);

Screen(window, 'FillRect', grey);
Screen(window, 'FillRect', black, FixationLoc);
[T2Timestamp]=Screen('Flip', window, (Cue_Time + .055)); 

Screen(window, 'FillRect', black, FixationLoc);
Screen('PutImage', window, StandGab, StandLoc);
Screen('PutImage', window, CompGab, CompLoc);
[T3Timestamp Begin_Time]=Screen('Flip', window, (Cue_Time + .11));%flips target window, presenting target to subject, and timestamps this to use for RT calculaions below
...
if Block == 0
    ...
    if (TotalTrial>=1) %starts saving data if subject has completed at least 1 trial
        ...
        Save = [Block; 0; Trial; Order(Trial); TrCueSide(Order(Trial)); TrStandOrient(Order(Trial)); TrCompContrast(Order(Trial)); TrStandSide(Order(Trial)); ...
    ...
    if (TotalTrial>=1) %starts saving data if subject has completed at least 1 trial
        ...
        Save = [Block; BlockOrder(Block); Trial; Order(Trial); TrCueSide(Order(Trial)); TrStandOrient(Order(Trial)); TrCompContrast(Order(Trial)); TrStandSide(Order(Trial)); ...
        Response(Trial); Accuracy(Trial); RT(Trial); T1Timestamp; T2Timestamp; T3Timestamp; T4Timestamp];
