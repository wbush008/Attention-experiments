...
FileName =  strcat(ID, '_DuoChrome.csv'); 
...
RunLength = 8; %repetitions of same cue color
RunsPerBlock = 6; %number of runs between breaks
trialcode = 1;
...
if TrCueColorNum(RandRunOrder(Run)) == 1
    ...
    Cue2_Color = grey;
    Cue3_Color = ColorArray(TrCueColor(mod(RandRunOrder(Run)+3,9)+1),:);
    Cue4_Color = ColorArray(TrCueColor(mod(RandRunOrder(Run)+6,9)+1),:);
else
    Cue1_Color = ColorArray(TrCueColor(mod(RandRunOrder(Run)+3,9)+1),:);
    Cue2_Color = ColorArray(TrCueColor(mod(RandRunOrder(Run)+6,9)+1),:);
    Cue3_Color = ColorArray(TrCueColor(RandRunOrder(Run)),:);
    Cue4_Color = grey;
end
    
TrDistractorColors = ColorArray;
RemoveTargs = [TrCueColor(RandRunOrder(Run)) TrCueColor(mod(RandRunOrder(Run)+3,9)+1) TrCueColor(mod(RandRunOrder(Run)+6,9)+1)];
...
for ItemPos = 1:CA_Number
    Item = TrDistractorColors(randi(6,1),:);
